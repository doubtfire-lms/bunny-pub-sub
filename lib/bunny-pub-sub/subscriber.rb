...
class Subscriber
  ...
  def start_subscriber(callback)
    ...
    begin
      ...
      queue.subscribe(manual_ack: true, block: true) do |delivery_info, properties, params|
...
def register_subscriber(subscriber_config,
  ...
  begin
    listener = Subscriber.new subscriber_config, results_publisher
    ServerException.publisher = results_publisher
    ClientException.publisher = results_publisher
    listener.start_subscriber(action)
